...
QueueHandle_t xQueueADCsemProc;
...
static void AFEC_pot_Callback(void) {
  ...
  xQueueSendFromISR(xQueueADCsemProc, &adc, &xHigherPriorityTaskWoken);
}
...
static void task_proc(void *pvParameters){
	...
	while (1) {
		if (xQueueReceive(xQueueADCsemProc, &(procAdc), 1000)) {
			soma += procAdc.value;
			n++;
			if(n==10){
				res = soma/n;
...
int main(void) {
  ...
  xQueueADCsemProc = xQueueCreate(100, sizeof(adcData));
  if (xQueueADCsemProc == NULL)
  printf("falha em criar a queue xQueueADC \n");
